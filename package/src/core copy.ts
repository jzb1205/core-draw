...
export class Topology {
    ...
    requestAnimationFrame(() => {
      ...
      switch (this.moveIn.type) {
        ...
        case MoveInType.ResizeCP:
          let activePens = this.activeLayer.pens || []
          if (activePens.length === 1 && activePens[0].xCP) { 
            if ([-270,-90,90,270].includes(activePens[0].rotate)) {
              pos.x = this.mouseDown.x //母线纵向拖拽
            }else{
              pos.y = this.mouseDown.y //母线横向拖拽
            }
          }
  ...
  private onmousedown = (e: MouseEvent) => {
    ...
    switch (this.moveIn.type) {
      ...
      case MoveInType.Nodes:
        ...
        this.activeLayer.clear();
        this.data.pens.forEach((item:Pen) => {
          if (this.moveIn.hoverNode && item.relativeId === this.moveIn.hoverNode.id) {
            this.activeLayer.pens.push(item)
            this.activeLayer.pens.push(this.moveIn.hoverNode)
  ...
  private onmouseup = (e: MouseEvent) => {
    if (this.hoverLayer.line && this.hoverLayer.line.visible && this.data.cellInfo) {   //放开鼠标左键绘制
        ...
        switch (json.anchorCount) {
          ...
          case 2:
            if (!reverseSymbolList.includes(this.data.cellInfo.sbzlx)) {  //变压器倒置
              ...
              this.hoverLayer.line.to.anchorIndex = 1
              this.hoverLayer.line.to.id = node.id
              this.hoverLayer.line.to.x = node.rotatedAnchors[1].x
              this.hoverLayer.line.to.y = node.rotatedAnchors[1].y
            }
            break;
    ...
    // if (this.hoverLayer.line && (!this.hoverLayer.line.from.id || !this.hoverLayer.line.to.id)) {
    //   this.data.pens = this.data.pens.filter(it=>it.id !== this.hoverLayer.line.id)
    // }
  ...
  paste() {
    ...
    let hanMLCopy = false   //复制是否包含母线或整站  否则只能挂在母线下
    if (this.clipboard.pens.length>0 && this.clipboard.pens[0].name === 'combine') {
      hanMLCopy = this.clipboard.pens[0].children.some(it => {
        return it.type ===0 && it.anchors.length>4
      });
    }
    ...
    for (const pen of this.clipboard.pens) {
      ...
      // if ((!hanMLCopy && this.lastHoverNode && this.lastHoverNode.rotatedAnchors && this.lastHoverNode.rotatedAnchors.length>4) || hanMLCopy) {
  ...
  calcCopyPen(pen:Pen,idMaps,relativeX,relativeY){
    if (pen instanceof Line) {
      ...
      //判断粘贴方向
      if (this.lastHoverNode) { //组合复制粘贴到母线上形成topo关系
        if ([0,180].includes(this.lastHoverNode.rotate)) {
          ...
          if (!pen.from.id) {  //from端链接母线
            ...
            pen.from.id = this.lastHoverNode.id
            pen.from.x = pen.to.y
            pen.from.y = this.lastHoverNode.rotatedAnchors[this.moveIn.hoverAnchorIndex].x
          }
          if (!pen.to.id) { //to端链接母线
            if (pen.from.x > pen.to.x) {
              this.pasteDirection = 'right'
          ...
          console.log(this.pasteDirection)
        }
      }
  ...
  //移动到指定点
  moveToAssignPoint(domId:string,id:string,option){
    let pens = this.data.pens || []
    if (pens.length === 0) {
      return
    }
    ...
    pens.map((item) => {
      /// 去掉上次样式变更
      if (item.strokeStyle == option.lastColor && item.lastColor) {
        item.strokeStyle = item.lastColor;
        item.fillStyle = item.lastColor;
      }

      /// 找到当前节点变更样式并移动至画布中心
      if (id == item.id) {
        item.lastColor = item.strokeStyle || item.fillStyle;
        item.strokeStyle = option.lastColor;
        item.fillStyle = option.lastColor;

        if (this.data.scale < option.scale) {
          this.scaleTo(option.scale);
        }
