...
var ActiveLayer = /** @class */ (function (_super) {
    ...
    function ActiveLayer(options, TID) {
        ...
        _this.rotateCPs = []; //旋转控制点
        _this.sizeCPs = []; //尺寸控制点
        _this.controlSize = 3; //控制点大小
        _this.pens = [];
        // nodes: Node[] = [];
        // lines: Line[] = [];
        _this.rotate = 0;
        ...
        index_1.Store.set(_this.generateStoreKey('LT:ActiveLayer'), _this);
        return _this;
    }
    ActiveLayer.prototype.calcControlPoints = function () {
        if (this.pens.length === 1 && this.pens[0] instanceof node_1.Node) {
            ...
            var rect = this.pens[0].rect;
            if (this.pens[0].xCP) {
                this.sizeCPs = [
                    new point_1.Point(rect.center.x - rect.width / 2, rect.center.y),
                    new point_1.Point(rect.center.x + rect.width / 2, rect.center.y)
                ];
                this.rotateCPs = [
                    new point_1.Point(this.pens[0].rect.x + this.pens[0].rect.width / 2, this.pens[0].rect.y - 15),
                    new point_1.Point(this.pens[0].rect.x + this.pens[0].rect.width / 2, this.pens[0].rect.y + rect.height / 2)
                ];
            }
            else {
    ...
    ActiveLayer.prototype.clear = function () {
        ...
        this.sizeCPs = [];
        this.rotateCPs = [];
        index_1.Store.set(this.generateStoreKey('LT:activeNode'), null);
    };
    ...
    ActiveLayer.prototype.updateLines = function (pens) {
        ...
        var lines = [];
        if (pens.length === 1 && pens[0] instanceof line_1.Line) {
            lines.push(pens[0]);
        }
        else {
            ...
            for (var _i = 0, _a = this.data.pens; _i < _a.length; _i++) {
                ...
                for (var _b = 0, nodes_2 = nodes; _b < nodes_2.length; _b++) {
                    ...
                    line.textRect = null;
                    index_1.Store.set(this.generateStoreKey('pts-') + line.id, null);
                    lines.push(line);
                }
            }
        }
        index_1.Store.set(this.generateStoreKey('LT:updateLines'), lines);
    };
    ...
    ActiveLayer.prototype.add = function (pen) {
        ...
        if (pen instanceof node_1.Node) {
            index_1.Store.set(this.generateStoreKey('LT:activeNode'), pen);
        }
    };
    ActiveLayer.prototype.setPens = function (pens) {
        ...
        this.pens = pens;
        if (this.pens.length === 1 && this.pens[0] instanceof node_1.Node) {
            index_1.Store.set(this.generateStoreKey('LT:activeNode'), this.pens[0]);
        }
    };
